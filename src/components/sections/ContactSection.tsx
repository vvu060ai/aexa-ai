...
import { Users, Mail, Phone, Building2, MessageSquare, Send, CheckCircle2, Loader2, Check, AlertCircle, Calendar, ArrowLeft, MessageSquarePlus } from 'lucide-react';
...
const CAL_LINK = 'https://cal.com/vishal-ai-49enge';
              ...
              <AnimatePresence mode="wait">
                ...
                {!isSent && !showBooking && (
                  ...
                  >
                    ...
                    <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
                      ...
                      <RippleButton
                        type="submit"
                        disabled={isSending || !name.trim() || !email.trim() || !idea.trim()}
                        rippleColor="rgba(255, 255, 255, 0.4)"
                        className={cn(
                          "mt-4 text-white font-black py-5 rounded-xl text-xl transition-all duration-500 shadow-lg border-none",
                          "bg-accent-primary shadow-accent-primary/20 hover:shadow-accent-primary/40 hover:-translate-y-1",
                          (isSending || !name.trim() || !email.trim() || !idea.trim()) && "cursor-not-allowed opacity-90"
                        )}
                      >
                        {isSending ? (
                          <>
                      ...
                      <div className="mt-2 flex flex-col items-center gap-3">
                        ...
                        <p className="text-xs font-medium text-foreground-muted/50 py-1 px-3 border border-border/30 rounded-full italic">
                          You will receive an email or call shortly.
                        </p>
                      </div>
                    </form>
                  </motion.div>
                ...
                {isSent && !showBooking && (
                  ...
                  >
                    ...
                    <div>
                      <h3 className="text-3xl font-bold mb-3">{contact.booking.successTitle}</h3>
                      <p className="text-foreground-muted text-lg">{contact.booking.successSubtitle}</p>
                    </div>
                    ...
                    <RippleButton
                      ...
                      className="mt-2 text-white font-black py-5 px-8 rounded-xl text-xl bg-accent-primary shadow-lg shadow-accent-primary/20 hover:shadow-accent-primary/40 hover:-translate-y-1 transition-all duration-500 border-none flex items-center gap-3"
                    >
                      <Calendar className="w-5 h-5" />
                      {contact.booking.ctaButton}
                    </RippleButton>
                    ...
                    >
                      ...
                      {contact.booking.sendAnotherText}
                    </button>
                  </motion.div>
                )}

                {/* State 3: Cal.com iframe */}
                {showBooking && (
                  <motion.div
                    key="booking"
                    initial={{ opacity: 0, y: 20 }}
                  ...
                  >
                    <div className="flex items-center justify-between mb-6">
                      <h3 className="text-2xl font-bold">{contact.booking.iframeTitle}</h3>
                      <button
                        type="button"
                      ...
                      >
                        ...
                        {contact.booking.backText}
                      </button>
                    </div>

                    <iframe
                      src={`${CAL_LINK}?theme=${resolvedTheme === 'dark' ? 'dark' : 'light'}&name=${encodeURIComponent(submittedName)}&email=${encodeURIComponent(submittedEmail)}&hide_event_type_details=1`}
                      className="w-full h-[500px] sm:h-[600px] border-0 rounded-xl"
